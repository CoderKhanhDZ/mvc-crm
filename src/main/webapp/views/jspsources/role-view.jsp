...
<div class="container-fluid">
	<div class="row bg-title">
		...
		<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
			<a href="<%=request.getContextPath() + UrlsController.URL_ROLE_ADD %>" class="btn btn-sm btn-success">Thêm mới</a>
		</div>
		<!-- /.col-lg-12 -->
	</div>
	<!-- /row -->
			...
			<div class="white-box">
				<div class="table-responsive">
					<div class="col-md-6">
						...
						<form method="get"
							action="<%=request.getContextPath() + UrlsController.URL_ROLE_VIEW %>">
					...
					<table class="table" id="example">
						...
						<tbody>
							<tr class="">
								<c:forEach var="role" items="${viewRoles}"
									varStatus="loop">
									<tr>
										...
										<td><a
											href="<%= request.getContextPath() + UrlsController.URL_ROLE_EDIT %>?id=${role.id}"
											class="btn btn-sm btn-primary ">Sửa</a> <a
											href="<%= request.getContextPath() + UrlsController.URL_ROLE_DELETE %>?id=${role.id}"
					...
					<nav aria-label="Page navigation example" align="right">
						<ul class="pagination">
								...
								<c:choose>
									<c:when test="${current_page == i}">
										<li class="page-item active"><a
											href="<%=request.getContextPath() +  UrlsController.URL_ROLE_VIEW %>?current_page=${i}&record_on_page=${record_on_page}&keyword_search=${keyword_search}">${i}</a></li>
									</c:when>
									<c:otherwise>
							...
							<c:if test="${current_page != totalPage}">
								<li class="next active"><a
									href="<%=request.getContextPath() +  UrlsController.URL_ROLE_VIEW %>?current_page=${current_page + 1 }&record_on_page=${record_on_page}&keyword_search=${keyword_search}">Next
										→ </a></li>
							</c:if>
						</ul>
					</nav>
